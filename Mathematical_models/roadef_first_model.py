...
model.addConstrs((risk_bar_t[t] == quicksum(risk_s_t[s,t] for s in range(scenario))*(1/scenario) for t in range(horizon)))

model.addConstrs((quicksum(y[s,t] for s in range(scenario)) >= tau*scenario for t in range(horizon)))
model.addConstrs((quicksum(y[s,t] for s in range(scenario)) <= tau*scenario +1 for t in range(horizon)))
...
#contrainte rajouter par Faicel
model.addConstrs( (quicksum(w[i,t1] for t1 in range(t, horizon)) >= z[i, t]*(delta_i_t[i][t]) for i in range(inter_numbers) \
for t in range(horizon)))
