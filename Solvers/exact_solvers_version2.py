...
class ExactSolvers:
  ...
  def create_mathematical_model(self,interventions_number,resources_number,horizon,list_z_indexes,scenarios,alpha,tau,computation_time, \
  delta_i_t,l_c_t,u_c_t,exclusions_list,r_c_i_t_t1,risk_s_i_t_t1,M,model_path,t_max):
   ...
   model.addConstrs((x[i, t]*(t + delta_i_t[i][t]) <= horizon for i in range(interventions_number) for t in range(t_max[i],horizon)))   
   model.addConstrs((x[i,t]*(t + 1) <= t_max[i] + 1 for i in range(interventions_number) for t in range(horizon)))
   
   model.addConstrs((z[key[0],t,t1] + z[key[1],t,t2] <= 1 for key in exclusions_list.keys() for t in exclusions_list[key] for t1 in range(delta_i_t[key[0]][t]) for t2 in range(delta_i_t[key[1]][t])))
   
  
   model.addConstrs(z[i,t,t1] >= x[i,t] for i in range(interventions_number) for t in range(t_max[i]+1) for t1 in range(t+1) if t>=t1)
   model.addConstrs((quicksum(z[i,t,t1]*r_c_i_t_t1[(i,res)][t][t-t1] for i in range(interventions_number) \
   for t1 in range(delta_i_t[i][t]) if (i,res) in r_c_i_t_t1 and t in r_c_i_t_t1[(i,res)] 
   and (t-t1) in r_c_i_t_t1[(i,res)][t]) >= l_c_t[res][t] for res in range(resources_number) for t in range(horizon)))
   
   model.addConstrs((quicksum(z[i,t,t1]*r_c_i_t_t1[(i,res)][t][t-t1] for i in range(interventions_number) \
   for t1 in range(delta_i_t[i][t]) if (i,res) in r_c_i_t_t1 and t in r_c_i_t_t1[(i,res)] \
   and (t-t1) in r_c_i_t_t1[(i,res)][t]) <= u_c_t[res][t] \
   for res in range(resources_number) for t in range(horizon)))
   
   model.addConstrs((risk_s_t[s,t] == quicksum(risk_s_i_t_t1[i][t][t-t1][s]*z[i,t,t1] for i in
   range(interventions_number) for t1 in range(delta_i_t[i][t]) if t in risk_s_i_t_t1[i] and (t-t1) in risk_s_i_t_t1[i][t]) \
    ...
    try:
      ...
      for i in range(interventions_number):
        for t in range(horizon):
          z = mathematical_model.getVarByName("x["+str(i)+","+str(t)+"]")
          if z.X != 0:
